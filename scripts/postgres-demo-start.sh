...
echo -e "${YELLOW}[1/5] Остановка существующих контейнеров...${NC}"
$COMPOSE_CMD -f "$COMPOSE_FILE" down 2>/dev/null || true

# Очистка старых данных (если требуется)
if [ "$1" == "--clean" ] || [ "$1" == "-c" ]; then
    echo -e "${YELLOW}[2/5] Очистка старых данных...${NC}"
    $COMPOSE_CMD -f "$COMPOSE_FILE" down -v
fi

# Запуск PostgreSQL
echo -e "${YELLOW}[2/5] Запуск PostgreSQL с демонстрационными данными...${NC}"
...
echo -e "${YELLOW}[4/5] Проверка демонстрационных данных...${NC}"
...
# Создание .env файла
echo -e "${YELLOW}[5/5] Создание .env файла...${NC}"
...
echo -e "  - Остановить: ${GREEN}docker-compose -f docker-compose.postgres.yml down${NC}"
