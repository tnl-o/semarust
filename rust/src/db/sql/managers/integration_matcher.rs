...
impl IntegrationMatcherManager for SqlStore {
    async fn get_integration_matchers(&self, project_id: i32, integration_id: i32) -> Result<Vec<IntegrationMatcher>> {
        match self.get_dialect() {
            SqlDialect::SQLite => {
                let rows = sqlx::query("SELECT * FROM integration_matcher WHERE integration_id = ? AND project_id = ?")
    ...
    async fn create_integration_matcher(&self, mut m: IntegrationMatcher) -> Result<IntegrationMatcher> {
        match self.get_dialect() {
            SqlDialect::SQLite => {
                let r = sqlx::query("INSERT INTO integration_matcher (integration_id, project_id, name, body_data_type, key, matcher_type, matcher_value, method) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
                    ...
                    .await.map_err(Error::Database)?;
                m.id = r.last_insert_rowid() as i32;
                Ok(m)
            }
            SqlDialect::PostgreSQL => {
    ...
    async fn update_integration_matcher(&self, m: IntegrationMatcher) -> Result<()> {
        match self.get_dialect() {
            SqlDialect::SQLite => {
                sqlx::query("UPDATE integration_matcher SET name=?, body_data_type=?, key=?, matcher_type=?, matcher_value=?, method=? WHERE id=? AND project_id=?")
    ...
    async fn delete_integration_matcher(&self, project_id: i32, _integration_id: i32, matcher_id: i32) -> Result<()> {
        match self.get_dialect() {
            SqlDialect::SQLite => {
                sqlx::query("DELETE FROM integration_matcher WHERE id=? AND project_id=?")
...
impl IntegrationExtractValueManager for SqlStore {
    async fn get_integration_extract_values(&self, project_id: i32, integration_id: i32) -> Result<Vec<IntegrationExtractValue>> {
        match self.get_dialect() {
            SqlDialect::SQLite => {
                let rows = sqlx::query("SELECT * FROM integration_extract_value WHERE integration_id=? AND project_id=?")
                    .bind(integration_id).bind(project_id)
                    .fetch_all(self.get_sqlite_pool().ok_or_else(|| Error::Other("SQLite pool not found".to_string()))?)
                    .await.map_err(Error::Database)?;
    ...
    async fn create_integration_extract_value(&self, mut v: IntegrationExtractValue) -> Result<IntegrationExtractValue> {
        match self.get_dialect() {
            SqlDialect::SQLite => {
                let r = sqlx::query("INSERT INTO integration_extract_value (integration_id, project_id, name, value_source, body_data_type, key, variable, value_name, value_type) VALUES (?,?,?,?,?,?,?,?,?)")
                    ...
                    .execute(self.get_sqlite_pool().ok_or_else(|| Error::Other("SQLite pool not found".to_string()))?)
                    .await.map_err(Error::Database)?;
                v.id = r.last_insert_rowid() as i32;
                Ok(v)
            }
            SqlDialect::PostgreSQL => {
    ...
    async fn update_integration_extract_value(&self, v: IntegrationExtractValue) -> Result<()> {
        match self.get_dialect() {
            SqlDialect::SQLite => {
                sqlx::query("UPDATE integration_extract_value SET name=?,value_source=?,body_data_type=?,key=?,variable=?,value_name=?,value_type=? WHERE id=? AND project_id=?")
    ...
    async fn delete_integration_extract_value(&self, project_id: i32, _integration_id: i32, value_id: i32) -> Result<()> {
        match self.get_dialect() {
            SqlDialect::SQLite => {
                sqlx::query("DELETE FROM integration_extract_value WHERE id=? AND project_id=?")
